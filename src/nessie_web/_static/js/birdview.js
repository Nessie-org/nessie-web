...
function updateBirdview(ws) {
	...
	/* Edges */
	ctx.strokeStyle = 'rgba(50,54,63,.9)';
	ctx.lineWidth = 0.8;
	ws.edges.forEach((e) => {
		const src = e.source,
			tgt = e.target;
	...
	validNodes.forEach((n) => {
		const nx = bx(n.x),
			ny = by(n.y);
		const nw = Math.max(4, n.w * s),
			nh = Math.max(3, n.h * s);
		ctx.fillStyle =
			n.id === ws.selectedId ? 'rgba(0,217,255,.1)' : '#181b22';
		ctx.strokeStyle = n.id === ws.selectedId ? '#00d9ff' : '#32363f';
		ctx.lineWidth = n.id === ws.selectedId ? 1.2 : 0.7;
	...
	ctx.strokeStyle = 'rgba(0,217,255,.7)';
	ctx.fillStyle = 'rgba(0,217,255,.04)';
	ctx.lineWidth = 1;
	ctx.setLineDash([3, 3]);
