...
function _ncx(n) {
	return n.tag === 'circle' || n.tag === 'ellipse'
		? n.x + n.rx
		: n.x + n.w / 2;
}
function _ncy(n) {
...
function loadGraph(ws, data) {
	...
	ws.nodes = [...nodesContainer.querySelectorAll('[node]')].map((el, i) => {
		...
		n.id =
			el.dataset.id || (data.nodes[i] && data.nodes[i].id) || String(i);
	...
	ws.zoom = d3
		...
		.on('zoom', (ev) => {
			...
			if (nodesContainer)
				d3.select(nodesContainer).attr('transform', ev.transform);
			if (edgesContainer)
				d3.select(edgesContainer).attr('transform', ev.transform);
	...
	/* ── Drag — mirrors the canonical D3 force-drag pattern ────── */
	const drag = d3
		.drag()
	...
	ws.nodes.forEach((n) => {
		d3.select(n.el)
			...
			.on('mouseover', (ev) => { ev.stopPropagation(); hlNode(ws, n.id); })
	...
	const avgSize =
		ws.nodes.reduce((s, n) => s + Math.max(n.w, n.h), 0) /
		(ws.nodes.length || 1);

	if (ws.sim) ws.sim.stop();
	ws.sim = d3
		.forceSimulation(ws.nodes)
		.force(
			'charge',
			d3.forceManyBody().strength(ws.simParams.charge).distanceMax(600),
		)
		.force(
			'collision',
			d3
		...
		.on('end', () => {
			updateBirdview(ws);
			// After initial settle, reduce forces so drag causes less
			// disturbance to the rest of the graph.
			ws.sim
				.force('charge', d3.forceManyBody().strength(ws.simParams.charge * 0.3).distanceMax(200))
...
function tick(ws) {
	...
	ws.edges.forEach((e) => {
		...
		e.el.setAttribute('x1', x1);
		e.el.setAttribute('y1', y1);
		e.el.setAttribute('x2', x2);
		e.el.setAttribute('y2', y2);
	});
...
function selectNode(ws, id) {
	...
	if (ws.id === activeWsId) {
		...
		if (id) {
			...
			document.getElementById('sb-sel-id').textContent = id;
...
function hlNode(ws, id) {
	...
	ws.nodes.forEach((n) => {
		n.el.style.opacity = conn.has(n.id) ? 1 : 0.2;
	});
	ws.edges.forEach((e) => {
		const s = e.source,
			t = e.target;
