...
function readLayout(ws) {
  ...
  return {
    ...
    settingsOpen: p.querySelector('.settings-body')?.classList.contains('open') || false,
...
function applyLayout(ws, layout) {
  ...
  if (layout.activeTab && layout.activeTab !== 'console') {
    p.querySelectorAll('.btab').forEach(t => t.classList.remove('active'));
    p.querySelectorAll('.tab-pane').forEach(t => t.classList.remove('active'));
    p.querySelector(`.btab[data-tab="${layout.activeTab}"]`)?.classList.add('active');
    p.querySelector(`.tab-pane[data-tab-id="${layout.activeTab}"]`)?.classList.add('active');
...
function restoreWorkspaceUiState(ws, saved) {
  ...
  if (typeof saved.simRunning === 'boolean') {
    ...
    ws.pane.querySelector('.sim-toggle').classList.toggle('on', ws.simRunning);
  }
